        ...
        cy.get('.TeamBubbles-modal_name')
          .then(($h3) => {
              ...
              //   console.log('this is the name 1: ' + nameText1)
                 cy.writeFile('export/modalContent.csv', nameText1, {flag:'a+'})
             })
        cy.get('.TeamBubbles-modal_content')
           .find('span').then(($s) => {
              ...
              //   console.log('this is the spanText 1: ' + spanText)
                 cy.writeFile('export/modalContent.csv', spanText, {flag:'a+'})
           })
    ...
    it('locate and extract 2', () => {
        ...
        cy.get('.TeamBubbles-modal_name')
          .then(($h3) => {
                     ...
                     cy.writeFile('export/modalContent.csv', nameText2, {flag:'a+'})
                 })
         cy.get('.TeamBubbles-modal_content')
             .find('span').then(($s) => {
                   const bio = $s.map((i, el) => Cypress.$(el).text())
        ...
        cy.get('.TeamBubbles-modal_name')
          .then(($h3) => {
                  ...
                  //   console.log('this is the name 3: ' + nameText3)
                     cy.writeFile('export/modalContent.csv', nameText3, {flag:'a+'})
                 })
            cy.get('.TeamBubbles-modal_content')
              .find('span').then(($s) => {
                    const bio = $s.map((i, el) => Cypress.$(el).text())
        ...
        cy.get('.TeamBubbles-modal_name')
          .then(($h3) => {
              ...
              //       console.log('this is the name 4: ' + nameText4)
                     cy.writeFile('export/modalContent.csv', nameText4, {flag:'a+'})
                 })
            cy.get('.TeamBubbles-modal_content')
              .find('span').then(($s) => {
                    const bio = $s.map((i, el) => Cypress.$(el).text())
